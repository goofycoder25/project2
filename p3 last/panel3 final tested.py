...
class Panel3(ttk.Frame):
    def __init__(self, parent):
        ...
        self.led_states = {}      # previous LED states for change detection
    ...
    def update_leds(self):
        ...
        def apply_led(canvas, led_item, sink_pin, label):
            state = is_on(sink_pin)
            prev = self.led_states.get(label)
            if state != prev:
                self.led_states[label] = state
                if prev is not None:  # skip logging the very first read on startup
                    if state:
                        self.log_event(f"{label} glowing")
                    else:
                        self.log_event(f"{label} stopped glowing")
            self.set_indicator_led(canvas, led_item, state)

        # LED above BTN 1 (logical 7)
        if self.led_btn1_canvas is not None:
            apply_led(self.led_btn1_canvas, self.led_btn1, LED1_SINK_PIN, "ACS PWR LED")

        # LED above BTN 2 (logical 8)
        if self.led_btn2_canvas is not None:
            apply_led(self.led_btn2_canvas, self.led_btn2, LED2_SINK_PIN, "0-ACS LED")

        # LED under BTN 4, label "1" (logical 15)
        if self.led_data_cv1_canvas is not None:
            apply_led(self.led_data_cv1_canvas, self.led_data_cv1, DATA_CV1_SINK_PIN, "DATA CV 1")

        # LED to the right, label "2" (logical 16)
        if self.led_data_cv2_canvas is not None:
            apply_led(self.led_data_cv2_canvas, self.led_data_cv2, DATA_CV2_SINK_PIN, "DATA CV 2")

        # ACS, CPLR, PWR CV 1..3 (logical 9..13)
        if self.led_acs_canvas is not None:
            apply_led(self.led_acs_canvas, self.led_acs, ACS_SINK_PIN, "ACS")

        if self.led_cplr_canvas is not None:
            apply_led(self.led_cplr_canvas, self.led_cplr, CPLR_SINK_PIN, "CPLR")

        if self.led_pwr_cv1_canvas is not None:
            apply_led(self.led_pwr_cv1_canvas, self.led_pwr_cv1, PWR_CV1_SINK_PIN, "PWR CV 1")

        if self.led_pwr_cv2_canvas is not None:
            apply_led(self.led_pwr_cv2_canvas, self.led_pwr_cv2, PWR_CV2_SINK_PIN, "PWR CV 2")

        if self.led_pwr_cv3_canvas is not None:
            apply_led(self.led_pwr_cv3_canvas, self.led_pwr_cv3, PWR_CV3_SINK_PIN, "PWR CV 3")
    ...
    def build_ui(self):
        ...
        # Use grid on main so left expands to fill all space beside the log
        main.columnconfigure(0, weight=1)
        main.columnconfigure(1, weight=0)
        main.rowconfigure(0, weight=1)

        left = ttk.Frame(main)
        left.grid(row=0, column=0, padx=(2, 0), pady=4, sticky="nsew")

        right = ttk.Frame(main)
        right.grid(row=0, column=1, padx=(4, 2), pady=4, sticky="ns")

        # -------------------------
        # BUTTON GRID + LED LAYOUT
        # -------------------------
        btn_grid = ttk.Frame(left)
        btn_grid.pack(anchor="w", pady=(4, 0), padx=(2, 0))
        ...
        for i in range(4):
            ...
            if i in (0, 1):
                ...
                led_frame.grid(row=0, column=0, pady=(0, 2))
        ...
        for i in range(3):
            col = ttk.Frame(btn_grid)
            col.grid(row=1, column=i, padx=8, pady=(12, 0))
        ...
        led1_col.grid(row=1, column=3, padx=8, pady=(12, 0), sticky="n")
        ...
        stack_col.grid(row=0, column=4, padx=(8, 0), sticky="n")
        ...
        led2_col.grid(row=1, column=4, padx=(8, 0), pady=(12, 0), sticky="n")
        ...
        data_cv_frame.grid(row=2, column=3, columnspan=2, pady=(4, 0))
        ttk.Label(
            data_cv_frame,
            text="DATA CV",
            font=("Arial", 8, "bold")
        ...
        stack_col2.grid(row=0, column=5, padx=(8, 0), sticky="n")
        ...
        # Right-hand log area — full height, fixed width=22
        ttk.Label(
            right,
            text="PANEL 3 LOG",
            font=("Arial", 9, "bold")
        ...
        self.log = tk.Text(right, width=22, font=("Arial", 8))
    ...
    def on_button_click(self, btn_id):
        if btn_id == 2:
            ...
            if not self.button_states[2]:
                ...
                label = BUTTON_LABELS.get(2, "BTN 2")
                self.log_event(f"{label} is ON")
            ...
            label = BUTTON_LABELS.get(btn_id, f"BTN {btn_id}")
            self.log_event(f"{label} is {'ON' if new_state else 'OFF'}")
    ...
    def auto_turn_off_btn2(self):
        if self.button_states.get(2):
            ...
            self.log_event(f"{BUTTON_LABELS.get(2, 'BTN 2')} is OFF (auto)")
